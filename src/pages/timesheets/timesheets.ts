...
/////BLOCO DE HORAS
function calculaHorasParte(horaInicial,horaFinal){
...
function diferencaHoras(horaInicial, horaFinal) {
  ...
  {
    let horasQueFaltamPraMeiaNoite = diferencaHoras(horaInicial, "24:00"); // chamada recursiva, há há!
    let totalHoras = somaHora(horasQueFaltamPraMeiaNoite, horaFinal);
    return totalHoras;
  }
}
...
function somaHora(horaInicio, horaSomada) {
  ...
  let minutosTotal = parseInt(horaIni[1], 10) + parseInt(horaSom[1], 10);
    ...
    let horaTotal = completaZeroEsquerda(horasTotal) + ":" + completaZeroEsquerda(minutosTotal);
    return horaTotal ;
}
/**
 ...
 */
function converteEmDiasHoras(totalHoras)
{
   ...
   if(horas < 24)
     return '00d ' + completaZeroEsquerda(horas) + 'h ' + completaZeroEsquerda(minutos) + 'm';
   
   let dias = Math.floor(horas/24); //Math.floor para forçar resultado inteiro
   horas = horas%24;
   return completaZeroEsquerda(dias) +'d ' + completaZeroEsquerda(horas) + 'h ' + completaZeroEsquerda(minutos) + 'm';
}
/**
